...
def residuals(params, all_calib_uvs, calib_objpoints):
    """
    Compute residuals between observed vs. expected (u,v) coordinates for a set of 
    points based on their 3D positions and camera parameters.
    ...
    all_extrinsics, all_intrinsics, calib_poses = \
        deserialize_params(params, all_calib_uvs.shape[0])
...
def bundle_adjustment_sparsity(all_calib_uvs):
    ...
    A : scipy.sparse.lil_matrix
        Sparse matrix representing the nonzero elements of the Jacobian matrix of the
        :py:func:`multicam_calibration.residuals` function.
    """
    ...
    for s in range(12): A[i, camera_ixs * 12 + s] = 1
    for s in range(6): A[i, n_cameras * 12 + frame_ixs * 6 + s] = 1
    return A


def serialize_params(all_extrinsics, all_intrinsics, calib_poses):
    """
    Serialize camera parameters and 3D calibration point positions into a flat array.
...
def bundle_adjust(all_calib_uvs, all_extrinsics, all_intrinsics, calib_objpoints,
                  calib_poses, n_frames=10000, outlier_pctl=95, **opt_kwargs):
    """
    ...
    and predicted (u,v) coordinates of calibration points. Frames with < 2 cameras 
    observing the calibration object are ignored. 
    ...
    predicted_uvs = predict_calib_uvs(
        all_extrinsics, all_intrinsics, calib_objpoints, calib_poses[use_frames])
    ...
    # sample frames
    if n_frames is None or n_frames > len(use_frames): n_frames = len(use_frames)
    else: use_frames = np.random.choice(use_frames, n_frames, replace=False)
    ...
    default_kwargs = dict(verbose=2, x_scale='jac', ftol=1e-4, method='trf', loss='soft_l1')
    default_kwargs.update(opt_kwargs)
    
    # run optimization
    result = least_squares(
        residuals, x0, jac_sparsity=A, **default_kwargs,
        args=(all_calib_uvs[:,use_frames], calib_objpoints))

    adjusted_extrinsics, adjusted_intrinsics, adjusted_calib_poses = \
          deserialize_params(result.x, n_cameras)
    
    return adjusted_extrinsics, adjusted_intrinsics, adjusted_calib_poses, use_frames, result
